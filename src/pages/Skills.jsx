...
export default function Skills() {
  const [mode, setMode] = React.useState("tech");
  const skills = mode === "tech" ? TECH_SKILLS : SOFT_SKILLS;

  const [activeIdx, setActiveIdx] = React.useState(0);

  // --- TM animation: 'open' -> 'closing' -> 'opening' -> 'open' (with overshoot) ---
  const [tmState, setTmState] = React.useState("open"); // 'open' | 'closing' | 'opening'
  const blinkTM = React.useCallback(() => {
    // prevent overlapping timelines
    setTmState("closing");
    const t1 = setTimeout(() => setTmState("opening"), 120);
    const t2 = setTimeout(() => setTmState("open"), 120 + 140);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
    ...
    <section className="mx-auto max-w-[1200px] md:max-w-[1320px] lg:max-w-[1400px] px-4 sm:px-6 lg:px-8">
      <div className="panel mt-6 sm:mt-10 p-6 sm:p-8 md:p-10 lg:p-12 xl:p-14 min-h-[70vh] md:min-h-[75vh] lg:min-h-[80vh] flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px,1fr] gap-6 sm:gap-8">
          ...
          <div className="panel relative p-4 flex items-center justify-center">
            <div
              ...
              style={{
                ...
                transition:
                  "transform 140ms cubic-bezier(.2,.9,.2,1), filter 120ms",
                willChange: "transform",
                filter: tmState === "closing" ? "brightness(0.95)" : "none",
              }}
            >
              {/* open sprite */}
              <img
                src={tmOpenImg}
                alt=""
                className={`absolute inset-0 w-full h-full object-contain select-none pointer-events-none transition-opacity ${
                  tmState === "closing" ? "opacity-0 duration-80" : "opacity-100 duration-140"
                }`}
                draggable={false}
                decoding="async"
              />
              ...
              <img
                ...
                alt="TM case"
                className={`absolute inset-0 w-full h-full object-contain select-none pointer-events-none transition-opacity ${
                  tmState === "closing" ? "opacity-100 duration-80" : "opacity-0 duration-140"
                }`}
                draggable={false}
                decoding="async"
              />
          ...
          <div className="panel p-4 sm:p-5 md:p-6 relative overflow-hidden bg-gradient-to-b from-sky-200/45 via-sky-100/50 to-sky-50">
            {/* header strip */}
                ...
                return (
                  <li key={s.name}>
                    <button
                      ...
                      className={`panel w-full text-left px-3 py-2 font-press text-[13px] tracking-wide transition ${
                        active
                          ? "ring-2 ring-sky-700 shadow-md"
